...
let _AG_ = parser (new_keyword "AG")
let _AF_ = parser (new_keyword "AF")
let _EG_ = parser (new_keyword "EG")
let _EF_ = parser (new_keyword "EF")
let _L_ = parser '<' | "⟨"
let _R_ = parser '>' | "⟩"
...
let parser form prio =
  ...
  | _AG_ f:(form PAtom)  when prio = PAtom -> cs1 cAll (cs1 globally f)
  | _EG_ f:(form PAtom)  when prio = PAtom -> cs1 cExi (cs1 globally f)
  | _AF_ f:(form PAtom)  when prio = PAtom -> cs1 cAll (cs1 eventually f)
  | _EF_ f:(form PAtom)  when prio = PAtom -> cs1 cExi (cs1 eventually f)
